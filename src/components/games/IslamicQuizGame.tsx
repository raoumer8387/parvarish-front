...
import { completeQuizSession, getQuizQuestions, type QuizQuestion, type QuizResponse } from '../../api/gamesApi';
...
export default function IslamicQuizGame() {
  ...
  const loadQuestions = async () => {
    try {
      const data = await getQuizQuestions('12-14', 2, 10);
      setQuestions(data);
      setLoading(false);
    } catch (e: any) {
      ...
      <div className="min-h-screen w-full bg-slate-50 flex items-center justify-center p-6 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] [background-size:16px_16px]">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl p-8 text-center space-y-6 border border-slate-100 animate-in zoom-in-95 duration-300 flex flex-col">
          ...
          <div className="flex justify-center">
            <div className="h-24 w-24 bg-amber-100 rounded-full flex items-center justify-center animate-bounce">
              <span className="text-4xl">🤲</span>
            </div>
          </div>

          <div className="space-y-2">
            <h1 className="text-3xl font-black text-slate-900">Quiz Complete!</h1>
            <p className="text-slate-600">Excellent knowledge of Islamic teachings!</p>
          </div>

          {/* Score Card */}
          <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
            <div className="text-sm font-bold text-slate-500 uppercase tracking-wider mb-2">Total Score</div>
            <div className="text-5xl font-black text-amber-600">{result.score.percentage}%</div>
          ...
          <Button 
            onClick={handleBack} 
            className="w-full h-12 text-base font-bold rounded-xl bg-amber-500 hover:bg-amber-600 text-white shadow-lg transition-transform hover:scale-[1.02] mt-auto"
            style={{ color: 'black' }}
          >
            Back to Games
          </Button>
